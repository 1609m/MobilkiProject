<?php
	...
	$anchor = "";
	if (isset($_POST['wynik']) && isset($_POST['rownanie'])) {
		$rownanie = $_POST['rownanie'];
		$wynik = $_POST['wynik'];
			...
			if ($wynik != "" && $rownanie != "") {
				...
				$result = $conn->query("SELECT tresc, klucz FROM zadania WHERE przedmiot_id = 1 AND typ = 1");
				$juzJest = false;
				while ($row = $result->fetch_assoc()) {
					if ($row['tresc'] == $rownanie && $row['klucz'] == $wynik)
						$juzJest = true;
				}
				
				if ($juzJest == false) {
					$conn->query("INSERT INTO zadania VALUES(NULL,1,1,'$rownanie','$wynik')");
				} else {
		...
		$wiadomosc = $_POST['wiadomosc'];
			...
			if (($wiadomosc != "") && isset($_POST['klasa'])) {
				...
				$conn->query("INSERT INTO ogloszeniazad VALUES(NULL,'$dane[1]','$klasa',1,'$wiadomosc')");
		...
		if ($tekst2 != "") {
			...
			if (!isset($_SESSION['er_dodaj2']) && $jestT) {
				...
				$conn = new mysqli($host, $user, $pass, $db);	
				$tekst2 = $_POST['tekst2'];
				$conn->query("INSERT INTO zadania VALUES(NULL, 1, 2, '$tekst2','')");
		
				$result = $conn->query("SELECT * FROM zadania WHERE przedmiot_id = 1 AND typ = 2 ORDER BY id DESC");
				while ($row = $result->fetch_assoc()) {
					$zadId = $row['id'];
					for ($i = 0; $i < $iloscP; $i++){
						$pytanie = $_POST[$i];
		...
		if (isset($_POST['klasaPaczka'])) {
				...
				$result = $conn->query("SELECT * FROM zadania WHERE przedmiot_id = 1 AND typ = 2");
				...
				if (!isset($_SESSION['er_paczka'])) {
					...
					$resultZad =  $conn->query("SELECT * FROM zadania WHERE przedmiot_id = 1 AND typ = 1");
					$j = 1;
					...
					$resultZad =  $conn->query("SELECT * FROM zadania WHERE przedmiot_id = 1 AND typ = 2");
					$j = 1;
					while ($rowZad = $resultZad->fetch_assoc()) {
						if (!empty($_POST["b".$j])) {
							$zadanieId = $rowZad['id'];
...
<body>
	...
	<?php $active = "z_mat"; require "header.php";	?>
	<main>
		<div class="container-fluid">
			<div class="row">
				...
				<div class="col-sm-8 my-3 mx-auto bg-secondary text-center text-light">
					...
					<?php
						if (isset($_POST['rownanie']) && !isset($_SESSION['er_dodaj1'])) {
					...
					<div id="a1" class="rozwin">
						<br>
						<form role="form" action="zadanieMat.php" method="post" id="post1">
					...
					<div id="c1" class="rozwin">
						<form role="form" action="zadanieMat.php" method="post" id="post1">
							...
							<?php
								...
								$conn = new mysqli($host, $user, $pass, $db);
								$dane = explode(",", $_SESSION['zalogowany']);
								$result = $conn->query("SELECT * FROM ogloszeniazad WHERE nauczyciel_id = '$dane[1]' AND przedmiot_id = '1' ORDER BY id DESC");
								$i = 0;
								$j = 1;
								while ($row = $result->fetch_assoc()) {
									$i++;
					...
					<div id="d1" class="rozwin">
						<br>
						<form action="zadanieMat.php" method="post">
							<label>Wprowadź równanie<br><input type="text" name="rownanie"></label><br>
							<label>Wprowadź wynik<br><input type="text" name="wynik"></label><br>
					...
					<div id="e1" class="rozwin">
						...
						<?php
							...
							while ($row = $result->fetch_assoc()) {
								$klucz = $row['klucz'];
								echo "<b>".++$i.". </b> &nbsp&nbsp".$row['tresc']." = <input type='text' size='10' style='text-align: center;' value='$klucz'><br><br>";
					...
					<div id="x1" class="rozwin">
							<?php
								if (!isset($_POST['tekst']) && !isset($_POST['iloscPytan']) && !isset($_POST['tekst2'])) {
									echo "<br><form action='zadanieMat.php' method='post'>";
					...
					<div id="z1" class="rozwin">
						...
						<form action='zadanieMat.php' method='post'>
							...
							<?php
								require_once "dbconnect.php";		
								$conn =  new mysqli($host, $user, $pass, $db);	
								$result = $conn->query("SELECT * FROM zadania WHERE typ = '1' AND przedmiot_id = '1'");
								$i = 1;
								while ($row = $result->fetch_row()) {
							...
							<?php
								require_once "dbconnect.php";		
								$conn =  new mysqli($host, $user, $pass, $db);	
								$result = $conn->query("SELECT * FROM zadania WHERE typ = '2' AND przedmiot_id = '1'");
								$i = 1;
								while ($row = $result->fetch_row()) {
